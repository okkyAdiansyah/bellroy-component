...
view model =
    div[]
        ...
        , viewControl model.product.productOptions
        , text model.product.productName ]

viewControl : List ProductOption -> Html Msg
viewControl option =
    div[]
        ( List.map(\item ->
        button[onClick (ChangeProductOption item.optionCode)][text item.optionColor])
        option
        )
